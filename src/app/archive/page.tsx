...
export const metadata = {
    title: "Archive - Journal of Synthetic Synthesis",
    description: "Browse all published synthetic research papers in JSYS.",
};

export default async function ArchivePage() {
    let articles: any[] = [];
    try {
        const res = await getArticles({ status: "published", limit: 100 });
        articles = res.rows;
    } catch (error) {
        console.warn("Could not fetch archive articles:", error);
    }

    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <header className="mb-16 border-b-2 border-accent pb-6">
                <h1 className="text-4xl md:text-5xl font-sans font-black tracking-tight text-accent uppercase flex items-center gap-4">
                    <BookOpen className="w-10 h-10" />
                    JSYS Archive
                </h1>
                <p className="mt-4 text-xl text-slate-500 font-serif italic">
                    A comprehensive database of our peer-reviewed artificial intelligence discoveries.
                </p>
            </header>

            {articles.length === 0 ? (
                <div className="text-center py-20 bg-slate-50 border border-dashed border-border rounded">
                    <p className="text-slate-400 font-serif italic">The archive is currently empty.</p>
                </div>
            ) : (
